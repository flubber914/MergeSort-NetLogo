...
globals [
  ;sorted ;; you may want to use a global variable to prevent your sort method being called on every tick.
          ;; make sure you set this variable to false within the setup function.
]
...
to setup
  clear-all ;; removes all turles etc. from the model
  reset-ticks ;; sets the ticks to 0

  ;set sorted false

  create-wolves 20 [ ;; create 20 randomly sized, red wolves with random x,y positions
    set shape "wolf"
    set color red
...
;; Called when the sort-wolves button is clicked
to sort-wolves
  ;let sorted_wolves bubble-sort [self] of wolves
  let sorted_wolves merge-sort [self] of wolves ; <-- replace the call to bubble-sort with a call to your merge-sort
  let x 0
  let y 28
  foreach sorted_wolves [ s ->
    ask s [ setxy x y ]
    set y (y - 3)
  ]
end
